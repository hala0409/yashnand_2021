...
class ERPfunctionComponent extends Component
{
	...
	public function get_signed_url($file) {
		$storage  = new StorageClient([
			...
			'keyFilePath' => WWW_ROOT .'/nghome/yashnand-erp-2021-b341de28ae6d.json',
		]);
	...
	public function mail_po_withrate($to,$view_po,$po_no,$po_project,$po_date)
	{
		...
		$erp_po_details = $erp_inve_po->get($view_po);
		$previw_list = $erp_inve_po_details->find()->where(['po_id'=>$view_po])->toarray();
		...
		$email_subject = "YashNand: Purchase Order (P.O.) Notification"; // The Subject of the email  
		$email_message = "Sir / Madam,<br>";
		$email_message .= "<p>Please find Purchase Order (P.O.) for your kind perusal.</p>";
		...
		$email_message .='<div id="scrolling-div">
		<table width="100%" border="1">
			<tbody>
				<!-- <tr align="center">
					<td colspan="13">'.$this->viewheader_po($erp_po_details['po_date'],$erp_po_details['bill_mode']).'
					</td>
				</tr> -->
				<tr align="center">
					<td colspan="13">
				...
				<tr>
					...
					<td colspan="5">'.$erp_po_details['po_no'].'</td>
					</td>
					  <td colspan="3"><strong>Date:</strong> </td>
					<td colspan="6">'.$this->get_date($erp_po_details['po_date']).'
					</td>
				</tr>
				 <tr>
					<td> <strong> Project Name: </strong></td>
					<td colspan="12"><strong>'.$this->get_projectname($erp_po_details['project_id']).'</strong>
					</td>
				</tr>';
                $email_message .='
			  <tr>
					<td><strong>P.O. Type:</strong></td>
					<td colspan="5">'. $type_name.'</td>
					 <td colspan="3"> <strong> Project Code: </strong></td>
					<td colspan="5"><strong>'.$this->get_projectcode($erp_po_details['project_id']).'</strong>
					
				</tr>';
				$bill_mode=($erp_po_details['bill_mode'] == 'mp')?'Madhya Pradesh':ucfirst($erp_po_details['bill_mode']);
				 $email_message .='<tr>
					<td><strong>Billing Mode:</strong></td>
					<td colspan="5">'.$bill_mode.'
					</td>
					<td colspan="3"><strong>Usage:</strong> </td>
					<td colspan="4">'.ucfirst(str_replace('_',' ',$erp_po_details['usage_name'])).'
					</td>
				</tr>
				
				<tr>
					<td><strong>Vendor Name:</strong></td>
					<td colspan="5">'.$this->get_vendor_name($erp_po_details['vendor_userid']).'
					</td>
					<td colspan="3"><strong>Vendor ID:</strong> </td>
					<td colspan="4">'.$erp_po_details['vendor_id'].' </td>
				</tr>
				<tr>
					<td><strong>Vendor Addresss:</strong></td>
					<td colspan="12">'. $erp_po_details['vendor_address'].'</td>
				</tr>
				<tr>
					<td><strong>PAN No</strong> </td>
					<td colspan="5">
						'. $this->get_vendor_detail($erp_po_details['vendor_userid'],'pancard_no').'
					</td>
					<td colspan="4"><strong>GST No </strong></td>
					<td colspan="3">
						'. $this->get_vendor_detail($erp_po_details['vendor_userid'],'gst_no').'
					</td>
				</tr>';
				$delivery_project=($erp_po_details['delivery_type'] == 'via')?$erp_po_details["delivery_project"]:$erp_po_details["project_id"];
                $delivery_date=($erp_po_details['delivery_date']!= NULL)?date("d-m-Y",strtotime($erp_po_details['delivery_date'])) :"";
				$email_message .='
				<tr>
					
					<td><strong>Contact No: (1)</strong> </td>
					<td colspan="5">
						'. $this->get_vendor_contact($erp_po_details["vendor_userid"],"one").'
					</td>
					<td colspan="4"><strong>Contact No: (2) </strong></td>
					<td colspan="3">
						'. $this->get_vendor_contact($erp_po_details["vendor_userid"],"two").'
					</td>
				</tr>
				<tr>
					<td><strong>Place of Delivery:</strong></td>
					<td colspan="12">
						'. $this->get_projectaddress($delivery_project).'
					</td>
				</tr>
				<tr>
					<td><strong>Site Contact No: (1)</strong> </td>
					<td colspan="5">'.$erp_po_details['contact_no1'] .'</td>
					<td colspan="3"><strong>Site Contact No: (2) </strong></td>
					<td colspan="4">'.$erp_po_details['contact_no2'].'</td>
				</tr>
				<tr>
					<td><strong>Payment Method:</strong></td>
					<td colspan="5">'. $erp_po_details['payment_method'].'</td>
					<td colspan="3"><strong>Mode of GST:</strong></td>
					<td colspan="4">'. $erp_po_details['mode_of_gst'].'</td>
				...
				<tr>
					...
					foreach($previw_list as $retrive_material){
						$description = ($retrive_material['description']!= "")?$retrive_material['description']:'NA';
					$get_materialitemcode= is_numeric($retrive_material['material_id'])?$this->get_materialitemcode($retrive_material['material_id']):$retrive_material['m_code'];
					$get_material_title=is_numeric($retrive_material['material_id'])?$this->get_material_title($retrive_material['material_id']):$retrive_material['material_id'];	
					$get_brandname=is_numeric($retrive_material['brand_id'])?$this->get_brandname($retrive_material['brand_id']):$retrive_material['brand_id'];
					$get_items_units=is_numeric($retrive_material['material_id'])?$this->get_items_units($retrive_material['material_id']):$retrive_material['static_unit'];
					$email_message .='
				<tr>
					<td style="display:none;">
						'. $get_materialitemcode.'
					</td>
					<td colspan="5">
						<center>
							('.$get_materialitemcode.')<br>
							'. $get_material_title.'
							('. $description.')
						</center>
					</td> 
					<td align="center">'.$get_brandname .'
					</td>
					<td>'. $retrive_material['quantity'].'</td>
					<td>'.$get_items_units .'
					</td>
					<td>'. $retrive_material['unit_price'].' </td>
					<td align="center">'. $retrive_material['discount'].' </td>
					<td align="center">'. $retrive_material['gst'].' </td>
					<td align="center">'. $retrive_material['amount'].' </td>
					<td align="center">'. $retrive_material['single_amount'].' </td>
				</tr>';
				...
				$email_message .='<tr>
					...
					<td id="total_po_amount" colspan="1" align="center"><b>'. $total_amount.'<b></td>
					<td></td>
				</tr>
				<tr>
				...
				<tr>';
				$taxes_duties=($erp_po_details["taxes_duties"]) ? 'checked' : '';
				$loading_transport=($erp_po_details["loading_transport"]) ? 'checked' : '';
				$unloading=($erp_po_details["unloading"]) ? 'checked' : '';
				$warranty=($erp_po_details["warranty"]) ? 'checked' : '';
				$email_message .='<tr>
					<td align="right"><input type="checkbox" name="vehicle" value="Bike"
							'. $taxes_duties.' disabled></td>
					<td colspan="11"><strong>All Taxes & Duties</strong></td>
				</tr>';
				$email_message .='
				<tr>
					<td align="right"><input type="checkbox" name="vehicle" value="Bike"
							'. $loading_transport.' disabled></td>
					<td colspan="11"><strong>Loading & Transportation - F. O. R. at Place of Delivery</strong>
					</td>
				</tr>
				<tr>
					<td align="right"><input type="checkbox" name="vehicle" value="Bike"
							'. $unloading.' disabled></td>
					<td colspan="11"><strong>Unloading</strong></td>
				</tr>
				<tr>
					<td align="right"><input type="checkbox" name="vehicle" value="Bike"
							'. $warranty.' disabled></td>
					<td colspan="11"><strong>Replacement Warrenty up to : &nbsp; &nbsp;
							&nbsp;<span>'. $erp_po_details["warranty"].'</span></strong> </td>
				</tr>';
				$email_message .='<tr>
				...
				<tr>
				<tr>';
			$email_message .="<td colspan='12'> 3) Manufacturer's Test Certificates are required for each batch of supply.
					</td>";
			$email_message .='<tr>
				<tr>
				...
				<tr><tr>';
				if($erp_po_details["bill_mode"] == "gujarat") {
				
					$email_message .='<td colspan="12"> <strong>Billing Address:</strong>214/5, Khyati Complex, Near Mithakhali
						Underbridge, Ellisbridge, Ahmedabad - 380006,Gujarat</td>';
				}else if($erp_po_details["bill_mode"] == "mp"){ 
					$email_message .='<td colspan="12"> <strong>Billing
							Address:</strong>'. $this->getmpbilladdress($erp_po_details["po_date"]).'
					</td>';
					 } else if($erp_po_details["bill_mode"] == "maharastra"){ 
						$email_message .='<td colspan="12"> <strong>Billing Address:</strong>F - 302, P. No. - 21, 22, Sumit
						Residency, Bhagyashree Ni Kharbi Road, Nagpur, Maharashtra - 440009.</td>';
					}else if($erp_po_details["bill_mode"] == "haryana"){ 
						$email_message .='<td colspan="12"> <strong>Billing Address:</strong>Porta Cabin No - 2, Pandit Deen Dayal
				...
				<tr>
					<td colspan="6"> <strong>PAN No.:</strong>
						'. $this->getstatepanno($erp_po_details["bill_mode"],$erp_po_details["po_date"]).'
					</td>
					<td colspan="6"> <strong>GST No.: '. $erp_po_details["gstno"].'</strong> </td>
				<tr>
					
				<tr>
					<td colspan="13">9)
						'. $this->getconditionofpowo($erp_po_details["po_date"],$erp_po_details["bill_mode"]).'
					</td>
				</tr>
				<tr>
					<td colspan="13"> 10) Payment will be done
						<strong>'. $erp_po_details["payment_days"].'</strong> days after date of
				...
				<tr>
					<td colspan="13">
						<pre
							style="background:none;border:0px;font-size:15px;padding:0;">'. $erp_po_details["remarks"].'</pre>
					</td>
				</tr>
				...
				<tr>
					<td valign="bottom" align="center" height="120" colspan="6">
						<font size="4">
							'. $this->get_user_name($erp_po_details["created_by"]).'<br><strong>
					...
					<td valign="bottom" align="center" height="120" colspan="6">
						<font size="4">
							<strong>'. $this->getletterheadsign($erp_po_details["po_date"],$erp_po_details['bill_mode']).'</strong>
						</font><br><br>';
						
					foreach($previw_list as $retrive_material)
					{
							$email_message .="<font size='4'>".$this->get_user_name($retrive_material['verified_by'])."</font>";
							break;
					}
			$email_message .='<br>
	...
	$email_message .= "---------------------------------------------------------------------------------------------------------------";
	$email_message .= "<p>Please <strong>Do Not Reply</strong> to this E-mail ID. This E-mail is system generated. For confirmation and/or queries, please contact:</p>";
	$email_message .= "<p><strong>Contact No: +91-8347555811 or +91-8347555616</strong></p>";
	$email_message .= "<p><strong>E-mail ID: </strong><a href='mailto:purchase@yashnand.com'>purchase@yashnand.com</a></p>";
	$email_message .= "---------------------------------------------------------------------------------------------------------------";
	...
	public function mail_po_withoutrate($to,$view_po,$po_no,$po_project,$po_date)
	{
		...
		$erp_inve_po_details = TableRegistry::get('erp_inventory_po_detail'); 
		$erp_po_details = $erp_inve_po->get($view_po);
		$previw_list = $erp_inve_po_details->find()->where(['po_id'=>$view_po,'currently_approved'=>1])->toarray();
		...
		<table width="100%" border="1">
			...
			$email_message .='<tr>
				...
				foreach($previw_list as $retrive_material)
				{
					...
					{
						echo "<font size='4'>".$this->get_user_name($retrive_material['verified_by'])."</font>";
						break;
					}
				}
			$email_message .='<br>
...
/*public function set_loan_outstanding($paid_amt,$user_id,$month,$year)
	...
	public function mail_salary_slip($slip_id,$date,$name,$user_email_id)
	{
		...
		$email_message="";
		$email_message .="<!DOCTYPE html><html data-lt-installed='true'><head><style>
		.text-center {
			text-align: center;
		}
		div{
		...
		*, *:before, *:after {
		...
		$email_message .= "Sir / Madam,<br>";
		$email_message .= "<p>Please check Payslip.</p>";
		$email_message .= "<p><strong>Personnel Name : {$name}</strong></p>";
		$email_message .= "<p><strong>Month & Year : {$date}</strong></p>";
		$email_message .= "<p>Thank You.</p>";
		
		
		$email_message .='<div class="block block-fill-white" style="border: 2px solid #aaa;float: left;width: 100%;padding:20px 5px;margin:30px 0px;">';
		
		$email_message .='<form class="form_horizontal formsize" id="user_form" action="" method="post" enctype=
		"multipart/form-data">';
		$email_message .='<div class="content controls">			
			<div class="form-row">
			...
			$email_message .='<div class="col-md-12 text-center">
					<h3 style="margin-bottom: -15px;"><strong>Pay Slip for '. $monthName ."-". date("y",strtotime($curr_date)).'</strong></h3>
				</div>			
			</div>
			...
			<div class="form-row">				
				...
				<span>'.$this->get_user_identy_number($data["user_id"]).'</span>
				
				</div>
				
				<div class="col-md-4"><strong>Full Name : </strong> 				
					<span>'. $this->get_full_user_name($data["user_id"]).'</span>
				</div>
			...
			$email_message .='<div class="form-row">			
				<div class="col-md-4"><strong>Employee At : </strong>
					<span>'. $this->get_projectname($data["employee_at"]).'</span>
				</div>
				
				<div class="col-md-4"><strong>Designation : </strong>
					<span>'. $this->get_category_title($data["designation"]).'</span>
				</div>
				
				<!--<div class="col-md-4"><strong>Date of Joining : </strong>
					<span>'. date("d-m-y",strtotime($this->get_user_joindate($data["user_id"]))).'</span>
				</div>-->

				<div class="col-md-4"><strong>Pay Type : </strong>
					<span>'. $this->get_pay_type($data["pay_type"]).'</span>
				</div>
		   </div>
		   <div class="form-row">
				...
				<div class="col-md-4"><strong>Payable Days : </strong>
					<span>'. $data["payable_days"].'</span>
				</div>-->
				<div class="col-md-4"><strong>Aadhar Card No : </strong>
					<span>'. $data["adhaar_card_no"].'</span>
				</div>
				
				<div class="col-md-4"><strong>PAN Card No : </strong>
					<span>'. $data["pan_card_no"].'</span>
				</div>
				
				<div class="col-md-4"><strong>Date of Birth : </strong>
					<span>'. date("d-m-y",strtotime($this->get_user_birthdate($data["user_id"]))).'</span>
					<input type="hidden" name="ac_no" value="'. date("d-m-Y",strtotime($this->get_user_birthdate($data["user_id"]))).'" >
				</div>
			</div>
		  <div class="form-row">				
				...
				<div class="col-md-4"><strong>C.T.C(Year) : </strong>
					<span>'. $this->get_user_ctc_year($data["user_id"]).'</span>
				</div>
				<!--
				<div class="col-md-2">Pay Rate</div>
				<div class="col-md-2"><input type="text" name="pay_rate" id="pay_rate" value="" class="form-control" /></div>
				-->				
		   </div>';
		  				
			$email_message .='<div class="form-row">';			
				
				$is_epf = $this->get_user_is_epf($data["user_id"]);
				if($is_epf != 'no') { 
				
				$email_message .='<div class="col-md-4"><strong>EPF No : </strong>
					<span>'. $this->get_user_epf_no($data["user_id"]).'</span>
					<input type="hidden" name="ac_no" value="'. $this->get_user_epf_no($data["user_id"]).'" >
				</div>
				
				<div class="col-md-4"><strong>UAN No : </strong>
					<span>'. $this->get_user_uan_no($data["user_id"]).'</span>
					<input type="hidden" name="bank" value="'. $this->get_user_uan_no($data["user_id"]).'" >
				</div>';
				 }
				
				$is_esi = $this->get_user_is_esi($data["user_id"]);
				if($is_esi != 'no') { 
			
				$email_message .='<div class="col-md-4"><strong>ESI No : </strong>
					<span>'. $this->get_user_esi_no($data["user_id"]).'</span>
					<input type="hidden" name="ac_no" value="'. $this->get_user_esi_no($data["user_id"]).'" >
				</div>';
				} 
			$email_message .='</div>
			...
			<!--<div class="form-row">
				<div class="col-md-4"><strong>PF Slip Ref. No. : </strong>
					<span>'. $this->get_user_pf_ref_no($data["user_id"]).'</span>
				</div>
			</div>-->
			...
			<div class="col-md-6">                
                <div class="block block-drop-shadow bg-primary">
                    ...
                    <div class="content" style="background: #F2DBDB;">
						...
						<tbody>
							<tr>
								<td>Basic Salary</td>
								<td align="center"></td>
								<td class="text-right" >'. $data["basic_pay_amount"].'</td>
							</tr>
							<tr>
								<td>Dearness Allowance (D.A.)</td>
								<td align="center"></td>
								<td class="text-right" >'. $data["da_amount"].'</td>
							</tr>
							<tr>';
							$email_message .='<td>House Rent Allowance (H.R.A.)</td> 
								<td align="center">
									
								</td>								
								<td class="text-right" id="acco_allowance_amt">'. $data["acco_amount"].'</td>
							</tr>
							<tr>
								<td>Conveyance Allowance</td> 
								<td align="center"></td>
								<td class="text-right" >'. $data["hra_amount"].'</td>
							</tr>
							<tr>
								...
								$email_message .='</td>					
								<td class="text-right" id="trans_allowance_amount">'. $data["transport_amount"].'</td>
							</tr>
							<tr>
								...
								<td class="text-right" id="food_allowance_amount">'. $data["food_amount"].'</td>
							</tr>
							<tr>
								<td>Medical Allowance</td>
								<td align="center"></td>
								<td class="text-right" >'. $data["medical_amount"].'</td>
							</tr>						
							...
							<tr>
								...
								<td class="text-right" id="mobile_allowance_amt">'. $data["mobile_amount"].'</td>
							</tr>
							<tr>
								<td>Other Allowance</td>
								<td class="text-center"></td>
								<td class="text-right">
									'. $data["other_allowance_amount"].'
								</td>
							</tr>
							<tr>
					...
					<div class="footer" style="background: #D99594;">
						<strong>TOTAL EARNINGS <span style="float:right" id="total_earning">'. $data["total_earning"].'</span></strong>
					</div>
                </div>
            </div>
                ...
                <div class="block block-drop-shadow">
                    ...
                    <div class="content" style="background:#FBD4B4">
						...
						<tbody>
							...
							$email_message .='<tr>
								<td>Loan Repayment / Advance &nbsp; <h5 style="display:inline;"><span class="label label-danger"> O/S : '. $data["loan_outstanding"].' </span></h5></td>
								<td class="text-right">
									'. $data["loan_payment"].'
								</td>
							</tr>
							<tr>
								<td>Mobile Bill Recovery';
									if($data["mobile_cug"] != 0)
										{
									$email_message .='&nbsp; <h5 style="display:inline;"><span class="label label-danger">Limit : '. $data["mobile_cug"].'</span></h5>';
									} 
								$email_message .='<td class="text-right">
							...
							<tr>';
							if($is_epf != 'no' && $is_esi != 'no'){ $css_data= "padding-bottom:167px";}else if($is_epf == 'no' && $is_esi == 'no'){ $css_data= "padding-bottom: 240px";}else if($is_epf != 'no' && $is_esi == 'no'){ $css_data= "padding-bottom: 203px";}else{$css_data= "padding-bottom: 203px";}
							if($is_epf != 'no' && $is_esi != 'no'){ $css_data1= "padding-bottom:166px";}else if($is_epf == 'no' && $is_esi == 'no'){ $css_data1= "padding-bottom: 239px";}else if($is_epf != 'no' && $is_esi == 'no'){ $css_data1= "padding-bottom: 202px";}else{$css_data1= "padding-bottom: 202px";}
								$email_message .='<td style="'.$css_data.'">Others</td>
								<td class="text-right" style="'.$css_data1.'">
									'. $data["others"].'
								</td>
							</tr>							
						</tbody>
						<!-- <tfoot>
							<tr>
								...
								<th class="text-right" id="total_deduction" style="padding-bottom: 19px;">'. $data["total_deduction"].'</th>
							</tr>							
						</tfoot> -->						
						</table>
					</div> 
					<div class="footer" style="background:#FABF8F;">
						<strong>TOTAL DEDUCTIONS<span style="float:right" id="net_pay">'. $data["total_deduction"].'</span></strong>
					</div>
					<div class="footer" style="background: #C0504D; color:#fff;">
						<h5 style="margin:6px 0 0 0;"><strong>NET PAY <span style="float:right" id="net_pay">'. $data["net_pay"].'</span></strong></h5>						
					</div>
                </div>                                               
	...
	$email_message .='</div></body><html>';
		...
		$email_to = array_filter($email_to, function($value) { return $value !== ''; });
		$email_to = array_filter($email_to, function($value) { return $value !== NULL; });
		//$email_to = "mansi@dasinfomedia.com";
		$headers = "From: ".$email_from;  
		/*$file = fopen($url,'rb');  
	...
	public function planningwo_approve_mail($to,$wo_id)
	{
		...
		$email_subject = "YashNand: Work Order (W.O.) Notification"; // The Subject of the email  
		$email_message = "Sir / Madam,<br>";
		$email_message .= "<p>Please find Work Order (W.O.) for your kind perusal.</p>";
		$email_message .= "<p><strong>W.O. No :</strong> {$wo_no}.</p>";
		$email_message .= "<p><strong>Project Name :</strong> {$wo_project}.</p>";
		$email_message .= "<p><strong>Party Name :</strong> {$wo_party}.</p>";
		$email_message .= "<p>Thank You.</p>";
			...
			$email_message .='<div id="scrolling-div">
				<table width="100%" border="1" >
					<tbody>
						...
						<tr>
							<td colspan="4" > <strong> Project Code: '. $this->get_projectcode($wo_data['project_id']).'</strong></td>
							<td colspan="9" > <strong> Project Name: '. $this->get_projectname($wo_data['project_id']).'</strong></td>
						</tr>
						<tr>
							<td><strong>Project Address::</strong></td>
							<td colspan="11">'. $this->get_projectaddress($wo_data['project_id']) .'</td>
						</tr>';
						$bill_mode=($wo_data['bill_mode'] == 'mp')?'Madhya Pradesh':ucfirst($wo_data['bill_mode']);
						$email_message .='<tr>
							<td colspan="2"><strong>Mode of Billing:</strong></td>
							<td colspan="5"> '. $bill_mode.'</td>
							<td colspan="3"><strong>Date:</strong> </td>
							<td colspan="4" >'. $this->get_date($wo_data['wo_date']) .'  </td>
							
						</tr>
						<tr>
							<td colspan="2"><strong>W. O. No:</strong></td>
							<td colspan="5"> '. $wo_data['wo_no'] .'</td>
							<td colspan="3"><strong>Yashnand GST No:</strong></td>
							<td colspan="4" > '. $wo_data['wo_no'] .'</td>					
						</tr>				
						<tr>';
		
						$party_userid=(is_numeric($wo_data['party_userid']))?$this->get_vendor_name($wo_data['party_userid']):$this->ERPfunction->get_agency_name_by_code($wo_data['party_userid']);
		
						$email_message .="<td><strong>Party's Name:</strong></td>";
						$email_message .='<td colspan="5" >'.$party_userid  .'</td>
							<td colspan="3" ><strong>Party ID:</strong> </td>
							<td colspan="4">'. $wo_data['party_id'] .' </td>
						</tr>
						<tr>
							<td><strong>Party Addresss:</strong></td>
							<td colspan="12" >'. $wo_data['party_address'] .'</td>
						</tr>
						<tr>
							<td><strong>Contact No: (1)</strong> </td>
							<td colspan="5">'. $wo_data["party_no1"].' </td>
							<td colspan="4" ><strong>Contact No: (2) </strong></td>
							<td colspan="3"> '. $wo_data["party_no2"] .'</td>
						</tr>
						<tr>
							<td><strong>E-mail Id:</strong></td>
							<td colspan="12" >'. $wo_data["party_email"].'</td>
						</tr>
						<tr>
							<td colspan="2" ><strong>Pan Card No:</strong> </td>
							<td colspan="4">'. $wo_data['party_pan_no'] .' </td>
							<td colspan="4" ><strong>GST No:</strong></td>
							<td colspan="3"> '. $wo_data['party_gst_no'] .'</td>
						</tr>
						<tr>
							<td colspan="2"><strong>Type of Contract:</strong></td>
							<td colspan="4" >'. $this->get_contract_title($wo_data['contract_type']).'</td>
							<td colspan="4"><strong>Payment Method:</strong></td>
							<td colspan="3" >'. $wo_data['payment_method'].'</td>
						</tr>
						<tr>
							<td colspan="2"><strong>Type of Work:</strong></td>
							<td colspan="12" >'. $this->get_planning_work_head_title($wo_data['work_type']).'</td>
						</tr>
						<tr>
						...
						$email_message .='<tr>
							<td>'. $retrive_material['contract_no'] .'</td>
							<td>
								'. $this->get_category_title($retrive_material['material_name']).' <br> 
								('. $retrive_material['detail_description'] .')
							</td>
							
							<td>'. $retrive_material['quentity'].'</td>
							<td>'. $retrive_material['quantity_upto_previous'].'</td>
							<td>'. $retrive_material['till_date_quantity'].'</td>
							<td>'. $retrive_material['unit'].'</td>
							<td>'. $retrive_material['unit_rate'] .' </td>
							<td>'. $retrive_material['amount'] .' </td>
							<td>'. $retrive_material['amount_till_date'] .' </td>
							<!--<td>'. $retrive_material['sgst'] .' </td>
							<td>'. $retrive_material['igst'].' </td>
							<td>'. $retrive_material['amount'] .' </td>-->
						</tr>';
						$total_amount += $retrive_material['amount'];
						...
						$email_message .='<tr>
								...
								<td id="total_po_amount">'. $total_amount .'</td>
								<td id="total_po_amount">'. $total_amount_till_date .'</td>
								
						</tr>';
						if($cgst){ 
						$email_message .='<tr>
								<td colspan="6" class="text-center"><b>CGST(%)</b></td>
								<td id="total_po_amount">'. $wo_data['cgst_percentage'] .'</td>
								<td id="total_po_amount">'. $wo_data['cgst'] .'</td>
								<td id="total_po_amount">'. $wo_data['till_date_cgst'] .'</td>
						</tr>';
						} 
						if($sgst){
						$email_message .='<tr>
								<td colspan="6" class="text-center"><b>SGST(%)</b></td>
								<td id="total_po_amount">'. $wo_data['sgst_percentage'] .'<b></td>
								<td id="total_po_amount">'. $wo_data['sgst'] .'</td>
								<td id="total_po_amount">'. $wo_data['till_date_sgst'] .'</td>
						</tr>';
						} 
						if($igst){
						$email_message .='<tr>
								<td colspan="6" class="text-center"><b>IGST(%)</b></td>
								<td id="total_po_amount">'. $wo_data['igst_percentage'] .'</td>
								<td id="total_po_amount">'. $wo_data['igst'] .'</td>
								<td id="total_po_amount">'. $wo_data['till_date_igst'] .'</td>
						</tr>';
						} 
						$email_message .='<tr>
								<td colspan="7" class="text-center"><b>Net Amount</b></td>
								<td id="total_po_amount">'. $wo_data['net_amount'] .'</td>
								<td id="total_po_amount">'. $wo_data['till_date_net_amount'] .'</td>
				...
				$css_data=($wo_data['contract_type'] == 1 || $wo_data['contract_type'] == 3 || $wo_data['contract_type'] == 4)?'':'display:none';
				$taxes_duties= ($wo_data["taxes_duties"]) ? "checked" : "";
				$guarantee=($wo_data["guarantee"]) ? "checked" : "";

				$email_message .='<tbody id="remark_1" style="'.$css_data.'">
						<tr>
							<td rowspan="21" valign="top"></td>
							
						</tr>
						<tr>
							<td align="right"><input type="checkbox" name="vehicle" value="Bike" '.$taxes_duties.' disabled ></td>
							<td colspan="11" ><strong>All Taxes & Duties</strong></td>
						</tr>
						
						<tr>
							<td align="right"><input type="checkbox" name="vehicle" value="Bike" '. $guarantee.' disabled ></td>
							<td colspan="11" ><strong>Guarantee up to  : &nbsp; &nbsp; &nbsp;<span>'. $wo_data["guarantee_time"].'</span></strong> </td>
						</tr>';
						...
						<tr>';
							...
							$email_message .='<td colspan="12" > <strong>Billing Address:</strong>214/5, Khyati Complex, Near Mithakhali Underbridge, Ellisbridge, Ahmedabad - 380006,Gujarat</td>';
							}else if($wo_data["bill_mode"] == "mp"){
							$email_message .='<td colspan="12" > <strong>Billing Address:</strong>'. $this->getmpbilladdress($wo_data["wo_date"]) .'</td>';
							}else if($wo_data["bill_mode"] == "maharastra"){
							$email_message .='<td colspan="12" > <strong>Billing Address:</strong>F - 302, P. No. - 21, 22, Sumit Residency, Bhagyashree Ni Kharbi Road, Nagpur, Maharashtra - 440009.</td>';
							}else if($wo_data["bill_mode"] == "haryana"){
							$email_message .='<td colspan="12" > <strong>Billing Address:</strong>Porta Cabin No - 2, Pandit Deen Dayal Upadhaya University of Health Science Campus Site, Gate No - 2 Kutail, Kutail Village, Karnal, Haryana - 134115.</td>';
							} 
						...
						<tr>
							<td colspan="6" > <strong>PAN No.:</strong> '. $this->getstatepanno($wo_data["bill_mode"],$wo_data["wo_date"]) .'  </td>
							<td colspan="6" > <strong>GST No.: '. $wo_data["gstno"].'</strong> </td>
						</tr>
						...
						<tr>
							<td colspan="13" >14) '. $this->getconditionofpowo($wo_data["wo_date"],$wo_data["bill_mode"]) .'</td>
						</tr>
						<tr>
							<td colspan="13" > 15) Payment will be done <strong>'. $wo_data["payment_days"].'</strong> days after date of delivery on site or bill submission which ever is later.</td>
						</tr>
						<tr>
							<td colspan="13" >
							<pre style="background:none;border:0px;font-size:15px;padding:0;">'. $wo_data["remarks"].'</pre>
							</td>
						</tr>
					</tbody>';
					$css_data1=($wo_data['contract_type'] == 5 || $wo_data['contract_type'] == 6 || $wo_data['contract_type'] == 7 || $wo_data['contract_type'] == 2)?'':'display:none';
		
					$taxes_duties=($wo_data["taxes_duties"]) ? "checked" : "";
					$loading_transport=($wo_data["loading_transport"]) ? "checked" : "";
					$unloading=($wo_data["unloading"]) ? "checked" : "";
					$guarantee=($wo_data["guarantee"] != "") ? "checked" : "";
					$warrenty=($wo_data["warrenty"] != "") ? "checked" : "";
		
					$email_message .='<tbody id="remark_2" style="'.$css_data1.'">
						<tr>
						...
						<tr>
							<td align="right"><input type="checkbox" name="vehicle" value="Bike" '. $loading_transport.' disabled></td>
							<td colspan="11" ><strong>Loading & Transportation - F. O. R. at Place of Delivery</strong></td>
						</tr>
						<tr>
							<td align="right"><input type="checkbox" name="vehicle" value="Bike" '.$unloading .' disabled ></td>
							<td colspan="11" ><strong>Unloading</strong></td>
						</tr>
						<tr>
							<td align="right"><input type="checkbox" name="vehicle" value="Bike" '.$guarantee .' disabled ></td>
						...
						<tr>
							<td align="right"><input type="checkbox" name="vehicle" value="Bike" '. $warrenty.' disabled ></td>
							<td colspan="11" ><strong>Replacement Warrenty up to  : &nbsp; &nbsp; &nbsp;<span>'. $wo_data["warrenty_time"].'</span></strong> </td>
						</tr>
						...
						<tr>
							<td colspan="13" > 17) '. $this->getconditionofpowo($wo_data["wo_date"],$wo_data["bill_mode"]) .'</td>
						</tr>
						<tr>
							<td colspan="13" > 18) Payment will be done <strong>'. $wo_data["payment_days"].'</strong> days after date of delivery on site or bill submission which ever is later.</td>
						</tr>
						<tr>
							<td colspan="13" >
							<pre style="background:none;border:0px;font-size:15px;padding:0;">'. $wo_data["remarks"].'</pre>
					...
					<tbody>
						<tr>
							...
							<font size="4"> ('. $this->get_user_name($wo_data["created_by"]).')<br><strong> Made By </strong></font> 
							</td>
							
							<td valign="bottom" align="center" height="120"><font size="4"><strong>'. $this->getletterheadsign($wo_data["wo_date"],$wo_data['bill_mode']) .'</strong><br><br>('. $this->get_user_name($first_approved_by).')<br><strong>Authorized Signatory</strong></font></td>
				...
				$email_message .= "---------------------------------------------------------------------------------------------------------------";
				$email_message .= "<p>Please <strong>Do Not Reply</strong> to this E-mail ID. This E-mail is system generated. For confirmation and/or queries, please contact:</p>";
				$email_message .= "<p><strong>Contact No: +91-8347555411</strong></p>";
				$email_message .= "<p><strong>E-mail ID: </strong><a href='mailto:planning@yashnand.com'>planning@yashnand.com</a></p>";
				$email_message .= "---------------------------------------------------------------------------------------------------------------";
		//Old Code for multiple attachment
		/*
		// $email_to = "priyal@dasinfomedia.com"; // Who the email is to  
	...
	public function mail_assetpo_withrate($to,$view_po,$po_no,$po_project,$po_date)
	{
		...
		$erp_po_details = $erp_asset_po->get($view_po); 
		$previw_list = $erp_asset_po_details->find()->where(array('po_id'=>$view_po));
		...
		$email_message = "Sir / Madam,<br>";
		$email_message .= "<p>Please find Asset Purchase Order (P.O.) for your kind perusal.</p>";
		$email_message .= "<p><strong>PO NO :</strong> {$po_no}.</p>";
		$email_message .= "<p><strong>Project Name :</strong> {$po_project}.</p>";
		$email_message .= "<p><strong>Party Name :</strong> {$po_party}.</p>";
		$email_message .= "<p>Thank You.</p>";
	...
	$email_message .='<div id="scrolling-div">
		<table width="100%" border="1" >
			<tbody>
				<tr align="center"><td colspan="13">'.$this->viewheader_po($erp_po_details['po_date'],$erp_po_details['bill_mode']).'</td></tr>
				<tr align="center"><td colspan="13"><h2><strong>Asset Purchase Order (PO)</strong></h2></td></tr>
				<tr>
					<td colspan="4" > <strong> Project Code: '.$this->get_projectcode($erp_po_details['project_id']).'</strong></td>
					<td colspan="9" > <strong> Project Name: '.$this->get_projectname($erp_po_details['project_id'])/*.', &nbsp;'.$this->ERPfunction->get_projectaddress($erp_po_details['project_id']).', &nbsp;'.$this->ERPfunction->get_projectcity($erp_po_details['project_id'])*/.'</strong></td>
				</tr>
				
				<tr>
					<td><strong>P. O. No:</strong></td>
					<td colspan="3" > '.$erp_po_details['po_no'] .'</td>
					<td colspan="2"><strong>Date:</strong> </td>
					<td colspan="3" >'.$this->get_date($erp_po_details['po_date']) .'  </td>
					<td colspan="1" ><strong>Time:</strong></td>
					<td colspan="3" >'.$erp_po_details['po_time'].'</td>
				</tr>
				<tr>
					<td><strong>Billing Mode:</strong></td>
					<td colspan="5" >'.$bill_mode .'</td>
					...
					$email_message .='<td colspan="4">'.$usage_type.' </td>
				</tr>';
				if($erp_po_details['usage_name'] == 'for_asset'){
				
				$email_message .='<tr>
					<td><strong>Asset Name:</strong></td>
					<td colspan="12" >'.$this->get_asset_name($erp_po_details['asset_id']) .'</td>
				</tr>';
				}
                $delivery_type=$this->get_projectaddress(($erp_po_details['delivery_type'] == 'via')?$erp_po_details["delivery_project"]:$erp_po_details["project_id"]);
				$email_message .='<tr>
					<td><strong>Vendor Name:</strong></td>
					<td colspan="5" >'.$this->get_vendor_name($erp_po_details['vendor_userid']) .'</td>
					<td colspan="3" ><strong>Vendor ID:</strong> </td>
					<td colspan="4">'.$erp_po_details['vendor_id'].' </td>
				</tr>
				<tr>
				...
				<tr>
					<td><strong>PAN No</strong> </td>
					<td colspan="5">'.$this->get_vendor_detail($erp_po_details['vendor_userid'],'pancard_no').' </td>
					<td colspan="4" ><strong>GST No </strong></td>
					<td colspan="3"> '.$this->get_vendor_detail($erp_po_details['vendor_userid'],'gst_no').'</td>
				</tr>
				<tr>
					
					<td><strong>Contact No: (1)</strong> </td>
					<td colspan="5">'.$this->get_vendor_contact($erp_po_details["vendor_userid"],"one").' </td>
					<td colspan="4" ><strong>Contact No: (2) </strong></td>
					<td colspan="3"> '.$this->get_vendor_contact($erp_po_details["vendor_userid"],"two") .'</td>
				</tr>
				<tr>
					<td><strong>Place of Delivery:</strong></td>
					<td colspan="12" >'.$delivery_type.'</td>
				</tr>
				<tr>';
					...
					<td colspan="4">'.$erp_po_details['contact_no1'].' </td>
					<td colspan="4" ><strong>Site Contact No: (2) </strong></td>
					<td colspan="3"> '.$erp_po_details['contact_no2'].'</td>
				</tr>
				<tr>
					<td colspan="2"><strong>Payment Method:</strong></td>
					<td colspan="11" >'.$erp_po_details['payment_method'].'</td>
					<!-- <td colspan="4" ><strong>Delivery Date</strong></td> -->
					<!-- <td colspan="3"> '.$delivery_date.'</td> -->
				</tr>
				...
				<tr>
					...
					$total_amount = 0;
					foreach($previw_list as $retrive_material){
					$material_id=is_numeric($retrive_material['material_id'])?$this->get_materialitemcode($retrive_material['material_id']):$retrive_material['m_code'];
					$material_title=is_numeric($retrive_material['material_id'])?$this->get_material_title($retrive_material['material_id']):$retrive_material['material_id'];
					$brand_id=is_numeric($retrive_material['brand_id'])?$this->get_brandname($retrive_material['brand_id']):$retrive_material['brand_id'];
					$units=is_numeric($retrive_material['material_id'])?$this->get_items_units($retrive_material['material_id']):$retrive_material['static_unit'];
				$email_message .='<tr>
					<td style="display:none;">'.$material_id .'</td>
					<td><center>('.$material_id.')<br>'.$material_title.'</center></td>
					<!--<td>'.$retrive_material['hsn_code'] .' </td>-->
					<td>'.$brand_id.'</td>
					<td>'.$retrive_material['quantity'].'</td>
					<td>'.$units.'</td>
					<td>'.$retrive_material['unit_price'].' </td>
					<td>'.$retrive_material['discount'].' </td>
					<td>'.$retrive_material['transportation'] .' </td>
					<td>'.$retrive_material['exice'].' </td>
					<td>'.$retrive_material['other_tax'] .' </td>
					<td>'.$retrive_material['amount'] .' </td>
					<td>'.$retrive_material['single_amount'] .' </td>
				</tr>';
				$total_amount += $retrive_material['amount'];
				}
				$taxes_duties=($erp_po_details["taxes_duties"]) ? "checked" : "";
				...
				$email_message .='<tr>
						...
						<td id="total_po_amount"><b>'.$total_amount .'<b></td>
						<td></td>
				</tr>
				<tr>
					<td rowspan="28" valign="top"><strong> Remarks/Note:</strong></td>
				...
				<tr>
					<td align="right"><input type="checkbox" name="vehicle" value="Bike" '.$warranty.' disabled ></td>
					<td colspan="11" ><strong>Replacement Warrenty up to  : &nbsp; &nbsp; &nbsp;<span>'.$erp_po_details["warranty"].'</span></strong> </td>
				</tr>';
				...
				<tr>
				<tr>';
				$email_message .="<td colspan='12' > 3) Manufacturer's Test Certificates are required for each batch of supply. </td>";
				$email_message .='<tr>
				<tr>
				...
				if($erp_po_details["bill_mode"] == "gujarat") {
					...
					$email_message .='<td colspan="12" > <strong>Billing Address:</strong>'.$this->getmpbilladdress($erp_po_details["po_date"]) .'</td>';
					}else if($erp_po_details["bill_mode"] == "maharastra"){
					$email_message .='<td colspan="12" > <strong>Billing Address:</strong>F - 302, P. No. - 21, 22, Sumit Residency, Bhagyashree Ni Kharbi Road, Nagpur, Maharashtra - 440009.</td>';
					}else if($erp_po_details["bill_mode"] == "haryana"){ 
					$email_message .='<td colspan="12" > <strong>Billing Address:</strong>Porta Cabin No - 2, Pandit Deen Dayal Upadhaya University of Health Science Campus Site, Gate No - 2 Kutail, Kutail Village, Karnal, Haryana - 134115.</td>';
				...
				$email_message .='<tr>
					<td colspan="6" > <strong>PAN No.:</strong> '.$this->getstatepanno($erp_po_details["bill_mode"],$erp_po_details["po_date"]) .'  </td>
					<td colspan="6" > <strong>GST No.: '.$erp_po_details["gstno"].'</strong> </td>
					
				<tr>
				<tr>
					<td colspan="13" >9) '.$this->getconditionofpowo($erp_po_details["po_date"],$erp_po_details["bill_mode"]) .'</td>
				</tr>
				<tr>
					<td colspan="13" > 10) Payment will be done <strong>'.$erp_po_details["payment_days"].'</strong> days after date of delivery on site or bill submission which ever is later.</td>
				</tr>

				<tr>
					<td colspan="13" >
					<pre style="background:none;border:0px;font-size:15px;padding:0;">'.$erp_po_details["remarks"].'</pre>
					</td>
				</tr>
				 
				<tr>
					<td valign="bottom"  align="center"  height="120" colspan="5"> 					
					<font size="4"> ('.$this->get_user_name($erp_po_details["created_by"]).')<br><strong> Prepared by </strong></font> 
					</td>';
					...
					$email_message .='<td valign="bottom" align="center" height="120" colspan="8"><font size="4"><strong>'.$this->getletterheadsign($erp_po_details["po_date"],$erp_po_details['bill_mode']) .'</strong></font><br><br>'; 
					foreach($previw_list as $retrive_material)
						{
							if($retrive_material['first_approved_by'])
							{
								$email_message .="<font size='4'>".$this->get_user_name($retrive_material['first_approved_by'])."</font>";
								break;
							}
						}
					$email_message .='<br>
	...
	public function mail_assetpo_withoutrate($to,$view_po,$po_no,$po_project,$po_date)
	{
		...
		$previw_list = $erp_inve_po_details->find()->where(array('po_id'=>$view_po,'currently_approved'=>1));
		$erp_po_details = $rmc_tbl->get($view_po);			
			...
			$email_message .='<div id="scrolling-div">
				<table width="100%" border="1" >
					<tbody>
						...
						<tr>
							<td colspan="4" > <strong> Project Code: '.$this->get_projectcode($erp_po_details['project_id']).'</strong></td>
							<td colspan="9" > <strong> Project Name: '.$this->get_projectname($erp_po_details['project_id']).'</strong></td>
						</tr>
						...
						<tr>
							...
							$email_message .='<br>
							<font size="4"><strong>Verified by</strong></font></td>
							<td valign="bottom" align="center" height="120" colspan="4"><font size="4"><strong>'.$this->getletterheadsign($erp_po_details["po_date"],$erp_po_details['bill_mode']) .'</strong></font><br><br>'; 
							foreach($previw_list as $retrive_material)
								{
									if($retrive_material['first_approved_by'])
									{
	...
	/*--------------- new function added For DB queries ---------------------*/
	public function vendor_group_new()
	{
		$erp_vendor_groups = TableRegistry::get("erp_vendor_groups");
		$groups = $erp_vendor_groups->find()->select(['id','code','title']);
		$vendor_group = array();
		foreach($groups as $group)
		{
			$vendor_group[$group->id] = array('id'=>$group->id,'code'=>$group->code,'title'=>$group->title);
	...
	public function rolewise_vendor_group_new($role)
	{
		if($role != 'deputymanagerelectric')
		{
			$erp_vendor_groups = TableRegistry::get("erp_vendor_groups");
			...
			// $vendor_group[15] = array('id'=>'15','code'=>'OT','title'=>'Others');
			$erp_vendor_groups = TableRegistry::get("erp_vendor_groups");
			$groups = $erp_vendor_groups->find()->select(['id','code','title'])->where(['id IN'=>['6','7','10','15']]);
			$vendor_group = array();
			foreach($groups as $group)
			{
				$vendor_group[$group->id] = array('id'=>$group->id,'code'=>$group->code,'title'=>$group->title);

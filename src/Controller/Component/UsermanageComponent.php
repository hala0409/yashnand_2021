	...
	public function fetch_approve_pr($user_id,$data = array())
	{
		...
		if(!empty($data))
		{
			...
			if($this->project_alloted($role)==1){  
				if(!empty($parts))
				{
					$sql = "select project_id,prno,contact_no1,contact_no2,pr_date,pr_id from  erp_inventory_purhcase_request where  approved_status = 0 AND ";
				}else{
					$sql = "select project_id,prno,contact_no1,contact_no2,pr_date,pr_id from  erp_inventory_purhcase_request where  approved_status = 0";
				}
			}
			else{
				if(!empty($parts))
		...
		if($this->project_alloted($role)==1){  
			$projects_ids = $this->users_project($user_id);
			if(!empty($projects_ids)){
				$result = $conn->execute('select project_id,prno,contact_no1,contact_no2,pr_date,pr_id from  erp_inventory_purhcase_request where  approved_status = 0 AND	project_id in ('.implode(',',$projects_ids ).')');	
			}else{
				$result=array();
			}
			...
			$result = $conn->execute('select project_id,prno,contact_no1,contact_no2,pr_date,pr_id from  erp_inventory_purhcase_request where  approved_status = 0');		
...
/* array("erphead"=>0,"erpmanager"=>0,"ceo"=>1,"MD"=>1,"contractadmin"=>1,"projectdirector"=>1,"constructionmanager"=>0,"materialmanager"=>0,"billingengineer"=>0,"accountant"=>0,"pmm"=>0,"accounthead"=>0,"senioraccount"=>0,"humanresource"=>0,"purchasehead"=>0,"purchasemanager"=>0) */
	...
	public function fetch_approve_pr_prtrack($user_id,$data = array())
	{
		...
		// debug($or);die;
		$result = $pr_tbl->find()->select(['pr_id','project_id','prno','contact_no1','contact_no2']);
		$result = $result->innerjoin(
			["erp_inventory_pr_material"=>"erp_inventory_pr_material"],
			["erp_inventory_purhcase_request.pr_id = erp_inventory_pr_material.pr_id"])
			->where($or)->select(['erp_inventory_pr_material.approved_date','erp_inventory_pr_material.material_id','erp_inventory_pr_material.brand_id','erp_inventory_pr_material.m_code','erp_inventory_pr_material.material_name','erp_inventory_pr_material.brand_name','erp_inventory_pr_material.static_unit','erp_inventory_pr_material.quantity','erp_inventory_pr_material.delivery_date','erp_inventory_pr_material.po_completed','erp_inventory_pr_material.purchase_remarks'])->order(["date(erp_inventory_pr_material.approved_date) DESC","erp_inventory_purhcase_request.project_id ASC"])->hydrate(false)->toArray();
		// debug($result);die;
		$new_array = array();
		foreach($result as $retrive)
		{
	...
	public function all_access_project_new($user_id)
	{
		$role = $this->get_user_role($user_id);
		$projects_ids = $this->users_project($user_id);
		$conn = ConnectionManager::get('default');
		...
		if($this->project_alloted($role)==1){  
			if(!empty($projects_ids)){ 
				$result = $conn->execute('select project_id,project_name,project_code from  erp_projects where project_id in ('.implode(',',$projects_ids ).') and actual_amount = 0');			
					
			}else{
				$result=array();
			}
		}else{
			$result = $conn->execute('select project_id,project_name,project_code from  erp_projects where actual_amount = 0');	
		}
		return $result;
	}
	public function access_project_ongoing_new($user_id)
	{
		$role = $this->get_user_role($user_id);
		$projects_ids = $this->users_project($user_id);
		$conn = ConnectionManager::get('default');
		...
		if($this->project_alloted($role)==1){ 
			if(!empty($projects_ids)){ 
				$result = $conn->execute('select project_id,project_name,project_code from  erp_projects where project_id in ('.implode(',',$projects_ids ).') and project_status = "On Going"');			
					
			}else{
				$result=array();
			}
		}else{
			$result = $conn->execute('select project_id,project_name,project_code from  erp_projects where project_status = "On Going"');	
		}
		return $result;
	}
	public function access_project_new($user_id)
	{
		$role = $this->get_user_role($user_id);
		$projects_ids = $this->users_project($user_id);
		$conn = ConnectionManager::get('default');
		...
		if($this->project_alloted($role)==1){ 
			if(!empty($projects_ids)){ 
				$result = $conn->execute('select project_id,project_name,project_code from  erp_projects where project_id in ('.implode(',',$projects_ids ).') and project_status != "Fully Completed"');			
					
			}else{
				$result=array();
			}
		}else{
			$result = $conn->execute('select project_id,project_name,project_code from  erp_projects where project_status != "Fully Completed"');	
		}
		return $result;
	}
}

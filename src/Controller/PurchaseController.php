...
class PurchaseController extends AppController
{
    ...
    public function addmaterial($material_id = null)
    {
        ...
        $projects = $erp_projects->find()->select(['project_id','project_name','project_code'])->order(array(
            'project_id Desc',
        ));
        $this->set('projects', $projects);

        $erp_material = TableRegistry::get('erp_material');
        $category = $this->ERPfunction->rolewise_vendor_group_new($this->role);
        $this->set('category', $category);

        $table_category = TableRegistry::get('erp_category_master');
        $unit_list = $table_category->find()->select(['cat_id','category_title'])->where(array('type' => 'unit'));
    ...
    public function viewmaterial($material_code = null)
    {
        ...
        $role = $this->role;
        if ($role == "erpoperator") {
            $projects = $this->Usermanage->all_access_project_new($this->user_id);
        } elseif ($role == "deputymanagerelectric") {
            $projects = $this->Usermanage->access_project_ongoing_new($this->user_id);
        } else {
            $projects = $this->Usermanage->access_project_new($this->user_id);
        }
        $this->set('projects', $projects);
        ...
        $groups = $this->ERPfunction->vendor_group_new();
    ...
    public function printporecord2($po_id)
    {
		...
		$this->set("erp_po_details",$data->toArray());
        
        $this->viewBuilder()->options([
			'pdfConfig' => [
				'orientation' => 'landscape',
				'filename' => 'Purchase_Order_' . $po_id
			]
		]);
    ...
    public function viewvendor()
    {
        ...
        $users_table = TableRegistry::get('erp_vendor');
         $user_list = $users_table->find()->select(['user_id','vendor_id','vendor_name','contact_no1','contact_no2','vendor_billing_address','email_id','attach_file','attach_label']);
    ...
    public function viewammendporecords($projects_id = null, $from = null, $to = null)
    {
        ...
        if ($role == "deputymanagerelectric") {
            ...
            $material_list = $erp_material->find()->select(['material_id','material_title','material_item_code'])->where(["material_id IN" => $material_ids]);
        } else {
            $material_list = $erp_material->find()->select(['material_id','material_title','material_item_code']);
        }
        $this->set('material_list', $material_list);

        $users_table = TableRegistry::get('erp_vendor');
        $vendor_department = $users_table->find()->select(['user_id','vendor_id','vendor_name']);
    ...
    public function prepareloi()
    {
        ...
        if ($this->role == "deputymanagerelectric") {
            $material_ids = $this->ERPfunction->get_deputymanagerelectric_material();
            $material_ids = json_decode($material_ids);
            $material_list = $erp_material->find()->select(['material_id','material_title'])->where(['material_id IN' => $material_ids, "material_code !=" => 17]);
        } else {
            $material_list = $erp_material->find()->select(['material_id','material_title'])->where(["material_code !=" => 17, "project_id" => 0]);
        }
        ...
        $erp_agency = TableRegistry::get('erp_agency');
        $agency_list = $erp_agency->find()->select(['agency_name','id']);
        $this->set('agency_list', $agency_list);

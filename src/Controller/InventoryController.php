...
class InventoryController extends AppController
{
		...
		{		
			if(isset($this->request->data["approve_list"]))
			{
				...
				if($this->role == "deputymanagerelectric")
				{
					$material_ids = $this->ERPfunction->get_deputymanagerelectric_material();
					$material_ids = json_decode($material_ids);
					$material_list = $erp_material->find()->select(['material_id','material_title','material_item_code'])->where(["project_id IN"=>[0,$project_id],'material_id IN'=>$material_ids]);
				}else{
					$material_list = $erp_material->find()->select(['material_id','material_title','material_item_code'])->where(["project_id IN"=>[0,$project_id]]);
				}
				$this->set('material_list',$material_list);
				$pr_tbl = TableRegistry::get("erp_inventory_purhcase_request");
	...
	{
		$image = WWW_ROOT . DS . 'img' . DS . 'logo' . DS .'header_new.jpg';
		// Read image path, convert to base64 encoding
		$imageData = base64_encode(file_get_contents($image));

		// Format the image SRC:  data:{mime};base64,{data};
		$img_data = 'data: ' . mime_content_type($image) . ';base64,' . $imageData;
		...
		$http_context = stream_context_create($arrContextOptions);
		$this->viewBuilder()->options([
			'pdfConfig' => [
				'orientation' => 'potrait',
				'paperSize' => [['c','A4','','' , 5 , 5 , 0 , 0 , 0 , 0],['potrait']],

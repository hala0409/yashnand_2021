...
class ERPfunctionHelper extends Helper
{
	...
	public function get_purchase_pr_materials($pid,$i=null,$project_id=null,$prDate) {
		$mt_tbl = TableRegistry::get("erp_inventory_pr_material");
		$data = $mt_tbl->find()->select(['due_date','approved','material_id','brand_id','m_code','material_name','brand_name','static_unit','purchase_first_approve','po_completed','pr_material_id','approved_date','purchase_remarks','quantity','delivery_date'])->where(["pr_id"=>$pid,"approved"=>0,"show_in_purchase"=>1])->hydrate(false)->toArray();
		if(!empty($data)) {
			// debug($data['due_date']);
			$table = "<table class='table-bordered' style='width:100%;border:0;'>";
			$user_id = $this->request->session()->read('user_id');
	...
	{		
		...
		// debug($or);die;
		$result = $po_tbl->find()->select(['po_id','project_id','po_no','po_purchase_type','po_date','po_time','vendor_userid']);
		$result = $result->innerjoin(
			...
			->where($or)->select(['erp_inventory_po_detail.id','erp_inventory_po_detail.approved','erp_inventory_po_detail.first_approved','erp_inventory_po_detail.material_id','erp_inventory_po_detail.brand_id','erp_inventory_po_detail.static_unit','erp_inventory_po_detail.quantity','erp_inventory_po_detail.delivery_date','erp_inventory_po_detail.verified','erp_inventory_po_detail.single_amount','erp_inventory_po_detail.amount','erp_inventory_po_detail.pr_mid'])->hydrate(false)->toArray();
		// debug($result);die;
			...
			//finalized-purchase-rate
			 $data['Records']=array(
				...
				'Payment Notification'=>array(
					'Send Notification'=>'inwardpayment',
				),
			);
			//purchase
	...
	{		
		...
		// debug($or);die;
		$result = $po_tbl->find()->select(['po_id','po_no','po_date','po_time','project_id','vendor_userid']);
		$result = $result->innerjoin(
			["erp_asset_po_detail"=>"erp_asset_po_detail"],
			["erp_asset_po.po_id = erp_asset_po_detail.po_id"])
			->where($or)->select(['erp_asset_po_detail.id','erp_asset_po_detail.first_approved','erp_asset_po_detail.verified','erp_asset_po_detail.material_id','erp_asset_po_detail.brand_id','erp_asset_po_detail.static_unit','erp_asset_po_detail.quantity','erp_asset_po_detail.single_amount','erp_asset_po_detail.amount'])->hydrate(false)->toArray();
		// debug($result);die;
